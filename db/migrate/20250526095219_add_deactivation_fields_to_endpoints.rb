class AddDeactivationFieldsToEndpoints < ActiveRecord::Migration[8.0]
  # `disable_ddl_transaction!` + `algorithm: :concurrently` is a Postgres-only
  # idiom. SQLite has no equivalent, so we drop the modifier and run the
  # index creation inside the normal DDL transaction.
  def change
    ...
    add_index :webhooks_outgoing_endpoints, [parent_association, :deactivated_at]
  end
end
