# Api::V1::ApplicationController is in the starter repository and isn't
# needed for this package's unit tests, but our CI tests will try to load this
# class because eager loading is set to `true` when CI=true.
# We wrap this class in an `if` statement to circumvent this issue.
if defined?(Api::V1::ApplicationController)
  class Api::V1::SegmentsController < Api::V1::ApplicationController
    account_load_and_authorize_resource :segment, through: :team, through_association: :segments

    # GET /api/v1/teams/:team_id/segments
    def index
    end

    # GET /api/v1/segments/:id
    def show
    end

    # POST /api/v1/teams/:team_id/segments
    def create
      if @segment.save
        render :show, status: :created, location: [:api, :v1, @segment]
      else
        render json: @segment.errors, status: :unprocessable_entity
      end
    end

    # PATCH/PUT /api/v1/segments/:id
    def update
      if @segment.update(segment_params)
        render :show
      else
        render json: @segment.errors, status: :unprocessable_entity
      end
    end

    # DELETE /api/v1/segments/:id
    def destroy
      @segment.destroy
    end

    private

    module StrongParameters
      # Only allow a list of trusted parameters through.
      def segment_params
        strong_params = params.require(:segment).permit(
          *permitted_fields,
          :name,
          :natural_language_source,
          # 🚅 super scaffolding will insert new fields above this line.
          *permitted_arrays,
          # 🚅 super scaffolding will insert new arrays above this line.
        )

        process_params(strong_params)

        strong_params
      end
    end

    include StrongParameters
  end
else
  class Api::V1::SegmentsController
  end
end
